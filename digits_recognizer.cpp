...
const DigitsRecognizer::TLayers DigitsRecognizer::DEFAULT_LAYERS = {
    DigitsRecognizer::IMAGE_H * DigitsRecognizer::IMAGE_W, 50, 20, 10
};
...
void DigitsRecognizer::loadNetwork(const TString& networkName, const TLayers& layers) {
    ...
    if (std::filesystem::exists(networkName)){
        try {
            network = Neural::LoadNetwork(networkName, layers);
...
void DigitsRecognizer::setTestingDataset(std::unique_ptr<Neural::Dataset>&& dataset) {
    this->testingDataset = std::move(dataset);
}

void DigitsRecognizer::setTrainingDataset(std::unique_ptr<Neural::Dataset>&& dataset) {
    this->trainingDataset = std::move(dataset);
}
...
double DigitsRecognizer::evaluateAccuracy(const std::vector<Neural::Sample>& samples) const {
    if (samples.empty()) return 0.0;

    size_t correct = 0;
    for (size_t i = 0; i < samples.size() && !stopRequested; ++i) {
        const auto prediction = network.predict(samples[i].input);
...
double DigitsRecognizer::trainEpoch(
    std::vector<Neural::Sample>& samples, double learningRate, std::mt19937& rng
) {
    ...
    for (size_t i = 0; i < samples.size() && !stopRequested; ++i) {
        ...
        for (unsigned j = 0; j < INNER_EPOCHS; ++j) {
...
void DigitsRecognizer::doLearning() {
    ...
    auto allSamples = trainingDataset->getAllSamples(30);
    ...
    for (size_t epoch = 0; epoch < MAX_EPOCHS && !stopRequested; ++epoch) {
        ...
        if (realAccuracy > bestTrainAccuracy + 0.001) {
            ...
            if (saveOnEachDigit) {
                Neural::SaveNetwork(network, networkName);
            }
        } else {
...
TestResult DigitsRecognizer::testNetwork() const {
    ...
    for (TDigit digitToCheck = 0; digitToCheck < 10 && !stopRequested; ++digitToCheck) {
        ...
        const auto samples = testingDataset->getSamplesForLabel(digitToCheck);
